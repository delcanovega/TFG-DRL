...
# Agent:
MEMORY_SIZE = 2000
HIDDEN_LAYER1_SIZE = 24
HIDDEN_LAYER2_SIZE = 42
MINIBATCH_SIZE = 32
...
class DQNAgent:
    ...
    def build_model(self):
        ...
        model.add(Dense(HIDDEN_LAYER1_SIZE, input_dim=self.state_space, activation='relu'))
...
class DoubleDQNAgent:
    ...
    Note:
        Don't instantiate this class directly
    """
    def __init__(self, state_space, action_space, tau=0.01):
        self.state_space = state_space
        self.action_space = action_space
        ...
        self.model_local = self.build_model()
    ...
    def predict_action(self, state):
        if random.uniform(0, 1) < self.exploration:
            ...
            action_value = self.model_local.predict(state)
            action = np.argmax(action_value)  # Exploit
        return action
    
    def bellman(self, reward, next_state):
        a = self.model_local.predict(next_state)
        a = np.argmax(a)
        return reward + self.discount_factor * np.amax(self.model_target.predict(next_state)[a])
    ...
    def update_target(self):
        self.model_target.set_weights(self.model_local.get_weights())
...
class RandomBatchAgentTwoBrains(DQNAgent):
    ...
    def optimal_action(self, state, mod):
        if mod == 1:
            action_value = self.apprentice_model.predict(state)
        else:
            print("¡¡¡¡¡¡¡¡¡Something gone wrong!!!!!!!!!")
            action_value = self.model.predic(state)
    ...
    def finalTrial(self, env):
        ...
        return acc_mentor, acc_apprentice
