...
PENATYFORSLOW = 0.999 #Each episode the reward recived will be reduce to reforce the fastest way to reach our goal 
...
#basada en la distancia al centro
def ourReward(state, penalty):
    # trampeamos la reward 
    # el absoluto sirve para que no penalice el subir la colina de la derecha (sin el absoluto devolveria valores negativos)
    reward = abs(state[0][0]) + 0.5

    # le damos un beneficio local que esperemos que le ayude (el agua del raton)
    #la segunda condicion hace que el premio se reciba cuando se encuentra SUBIEDO la colina de la derecha
    if reward >= 0.5 and state[0][1] > 0:
        reward += 1
...
def simulate(env, agent, use_apprentice=False):
    ...
    for i in range(EPISODES):
        ...
        while not done:
            ...
            reward = ourReward(state, penalty)
            penalty *= PENATYFORSLOW

            #print(state[0], ";\t Recompensa EP", reward, ";\t  Acumulada ", acc_reward)
