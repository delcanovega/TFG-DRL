...
class RandomBatchAgentTwoBrainsBestSave(DQNAgent):
	...
	def lastFight(self, env):
        ...
        for _ in range(BESTOF):
            reward_mentor = self.test(env, self.model)
            reward_apprentice = self.test(env, self.apprentice_model)
			      reward_best = self.test(env, self.best_model)

            # print("Round {}: mentor {} - apprentice {}".format(i + 1, reward_mentor, reward_apprentice))
            total_reward_mentor += reward_mentor
            total_reward_apprentice += reward_apprentice
			      total_reward_best += reward_best
			      act_mentor.append(reward_mentor)
			      act_apprentice.append(reward_apprentice)
			      act_best.append(reward_best)

        
        print("FINAL RESULT: MENTOR {} - APPRENDICE {} - BEST {}".format(total_reward_mentor, total_reward_apprentice, total_reward_best))

        return total_reward_mentor, total_reward_apprentice, total_reward_best, act_mentor, act_apprentice, act_best
