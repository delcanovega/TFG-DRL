...
from drl_agent_6 import RandomBatchAgentTwoBrains

##Grafica nueva velocidad 2 brains adadelta vs adam
...
def simulate(env, agent, use_apprentice=False):
    ...
    performance, scores, real_scores, maxVel, maxPos, stepsList, velAcc, posAcc, competes, apprenticeScores = [[] for _ in range(10)]
    ...
    for i in range(EPISODES):
        ...
        while not done:
            ...
            state = next_state
            acc_reward += reward
        apprenticeReward = subSimulate(env, agent)
    ...
    acc_mentor, acc_apprentice = agent.finalTrial(env)

    return scores, real_scores, maxVel, maxPos, stepsList, velAcc, posAcc, performance, competes, acc_mentor, acc_apprentice, apprenticeScores
...
def saveScores(scores, real_scores, maxVel, maxPos, velAcc, posAcc, performance, competes, acc_mentor, acc_apprentice, apprenticeScores):
...
def loadScores():
    ...
    return scores, real_scores, maxVel, maxPos, velAcc, posAcc, performance, competes,  bestchangedPos, bestchangedVal, acc_mentor, acc_apprentice, acc_best, apprenticeScores

def create_mega_plot(scores, real_scores, maxVel, maxPos, velAcc, posAcc, performance, competes, acc_mentor, acc_apprentice, apprenticeScores):
...
if __name__ == '__main__':
    ...
    scores, real_scores, maxVel, maxPos, stepsList, velAcc, posAcc, performance, competes, acc_mentor, acc_apprentice, apprenticeScores = simulate(env,
     RandomBatchAgentTwoBrains(env.observation_space.shape[0], env.action_space.n))
    saveScores(scores, real_scores, maxVel, maxPos, velAcc, posAcc, performance, competes, acc_mentor, acc_apprentice, apprenticeScores)
    ...
    plt = create_mega_plot(scores, real_scores, maxVel, maxPos, velAcc, posAcc, performance, competes, acc_mentor, acc_apprentice, apprenticeScores)
